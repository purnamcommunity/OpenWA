...
14 install-time patches (29.3). If it exists upstream or in OpenWA, it has a row here.
...
- **29.4 — the OpenWA contract view.** Rows are the 120 `IWhatsAppEngine` methods; use it to see
...
instance behind the neutral `IWhatsAppEngine` interface (120 methods +
...
flowchart LR
    subgraph OpenWA["OpenWA"]
        ...
        SVC --> IF["IWhatsAppEngine - 120 methods"]
        IF --> WA["WhatsAppWebJsAdapter"]
        IF --> BA["BaileysAdapter"]
        SVC --> STORE["OpenWA-side stores"]
    end
    WA --> WLIB["whatsapp-web.js 1.34.7<br/>+ 12 OpenWA patches"]
...
OpenWA ships fourteen exact, self-disabling source transforms over the installed engines. Each runs at
...
This is the patch visibility the matrix cells refer to. Every patch is engine-specific (12 on wwjs,
...
| 🔧¹⁴ media send id          | `sendImageMessage`, `sendVideoMessage`, `sendAudioMessage`, `sendDocumentMessage` on **wwjs**. Without it every media send fails in the page with an opaque 500 while text sends work, which makes the failure look like a problem with the file.                                                                                                      |
...
## 29.4 Full capability matrix — the OpenWA contract view (120 methods)
...
**Totals:** 120 methods → 240 adapter cells: **208 ✅, 32 ❌** (2 adapter-gaps, 23
...
- **120** interface methods → **240** adapter cells: **208 ✅** / **32 ❌** (2 adapter-gaps, 23
  library-limitations, 0 uncertain), spanning **24** methods.
- Of the 208 ✅ cells, **10 wwjs cells carry an explicit patch dependency** (4 × 🔧² status send,
...
- **14** install-time patches (12 whatsapp-web.js + 2 Baileys), all exact and self-disabling.
