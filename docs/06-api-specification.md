...
#### POST /api/sessions/:sessionId/calls/audio-token

Mint a single-use token for a browser to open this session's audio socket. The gateway's headless
Chromium has no microphone and nobody sitting at it, so call audio is carried by an operator's
browser over the `/voip-audio` socket.io namespace. That browser must never hold the API key: the
caller mints a token here and hands the browser only the token, which is bound to this session,
buys exactly one connection, and expires unredeemed after about a minute.
...
| Name      | Type   | Description                      |
| --------- | ------ | -------------------------------- |
| sessionId | string | Session ID the token is bound to |

**Request body** — none.

**Response** `200` — `{ "token": "…", "expiresInSeconds": 60 }`

**Errors:** `401` missing/invalid `X-API-Key` · `403` key lacks OPERATOR role · `503` this gateway has no VoIP audio device (`VOIP_AUDIO_ENABLED` is not set) — raised at mint time rather than at connect, so the caller still has time to fall back to its relay path
...
| `call.accepted` / `call.rejected` / `call.missed` | A ringing call ended — answered, declined, or never picked up. **Baileys only**: the whatsapp-web.js adapter raises the ring (`onCall`) and no outcome. The call's state does reach the page payload there, because 🔧¹⁰ forwards it (docs/29), but nothing maps it to an outcome, so the gap is in the adapter and not in what the engine can see. On that engine, poll `GET /sessions/:sessionId/calls/state` to tell a ringing call from a connected one | `{ sessionId, callId, from, outcome, isVideo, isGroup, timestamp }` — `callId` matches the `call.received` that preceded it, so the pair can be correlated. The engines report _what_ happened, never _who_ did it: an accept can come from any linked device. An outcome is only sent for a call this session saw ring, and offline-replayed signalling for calls that ended while disconnected is dropped. WhatsApp's `terminate` is deliberately unmapped — it covers both a caller hanging up before answer and either side ending an answered call, with nothing to tell them apart |
